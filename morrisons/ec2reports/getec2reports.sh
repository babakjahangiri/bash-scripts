...
###############################################################################
# The script get all ec2 list from the given aws accounts and create a CSV repo$
# and save the  CSV file in the given S3 bucket
# by Babak Jahangiri (babak.jahangiri@morrisonsplc.co.uk)
#################################################################################

accfile='awsacc.txt'
regionfile='awsregions.txt'
csvfile='ec2_reports.csv'
s3bucket='mori-nonprod-reports'
csvfile='ec2-reports.csv'
...
distacc="nonprod-infra"
    ...
    echo "**************************************************"
    echo "*                                                *"
    echo "*               Listing All EC2s                 *"
    echo "*                                                *"
    echo "**************************************************"


    #make the file header & clear file
    #echo '{ "header":[ "aws account", "region", "ARN", "InstanceId", "InstanceType", "ImageId", "Private IP", "Private DNS", "Public IP", "Public DNS", "Subnet Id", "VPC Id", "Name", "Status"]}' | jq -r ' .header | @csv' > $csvfile 
    echo '{ "header":[ "Account", "Region" , "InstanceId", "InstanceType", "ImageId", "Private IP", "Private DNS", "Public IP", "Public DNS", "Subnet Id", "VPC Id", "Name", "Status"]}' | jq -r ' .header | @csv' > $csvfile 
     ...
     do
          ...
          echo "can not read an empty account"
               ...
               do
                    ...
                    instanceslist=$(aws ec2 describe-instances --query "Reservations[*].Instances[*].{InstanceId:InstanceId,
